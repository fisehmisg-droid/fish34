...
- FIREBASE_KEY_PATH: Path to Firebase service account JSON (or set FIREBASE_KEY content)
...
- firebase_key.json (secure; add via Railway secret file or env)
...
- Add environment variables in the Railway project settings (secrets). Set `FIREBASE_CREDENTIALS` to the full Firebase service account JSON string, plus `BOT_TOKEN`, `ADMIN_TOKEN`, `ADMIN_USER_ID`, `CHAPA_SECRET_KEY`, `WEBHOOK_SECRET`, `BASE_WEB_URL`, and `PUBLIC_BOT_USERNAME`.
...
- Use a strong `ADMIN_TOKEN` and never commit `firebase_key.json` or secret keys to git.
